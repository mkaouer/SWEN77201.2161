...
public class StargazerService extends GitHubService {
	...
	protected PagedRequest<UserStargazer> createStargazerRequestWithCreatedDate(
			IRepositoryIdProvider repository, int start, int size)
	{
		String id = getId(repository);
		PagedRequest<UserStargazer> request = createPagedRequest(start, size);
		StringBuilder uri = new StringBuilder(SEGMENT_REPOS);
		uri.append('/').append(id);
		uri.append(SEGMENT_STARGAZERS);
		request.setUri(uri);
		request.setType(new TypeToken<List<UserStargazer>>() {
		}.getType());
		return request;
	...
	public List<UserStargazer> getStargazersWithCreatedDate(IRepositoryIdProvider repository)
			throws IOException {
		PagedRequest<UserStargazer> request = createStargazerRequestWithCreatedDate(repository, PAGE_FIRST, PAGE_SIZE);
		return getAll(request);
	}
	...
	public PageIterator<UserStargazer> pageStargazersWithCreatedDate(IRepositoryIdProvider repository)
	{
		return pageStargazersWithCreatedDate(repository, PAGE_SIZE);
	...
	public PageIterator<UserStargazer> pageStargazersWithCreatedDate(IRepositoryIdProvider repository,
			int size) {
		return pageStargazersWithCreatedDate(repository, PAGE_FIRST, size);
	}
	...
	public PageIterator<UserStargazer> pageStargazersWithCreatedDate(IRepositoryIdProvider repository,
			int start, int size) {
		PagedRequest<UserStargazer> request = createStargazerRequestWithCreatedDate(repository, start, size);
		return createPageIterator(request);
	}
